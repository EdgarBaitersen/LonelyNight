...
public class InventoryController_craft : MonoBehaviour {
	...
	void DragItems(){
		if(Input.GetMouseButtonDown(0)){
			...
			if(raycastResults.Count > 0) {
                if (raycastResults[0].gameObject.GetComponent<Item_craft>()) {
                    draggedItem = raycastResults[0].gameObject;
                    draggedItemParent = draggedItem.transform.parent;
                    draggedItem.transform.SetParent(UIManager_craft.instance.canvas);
                }
		...
		//Item follows mouse
		if(draggedItem != null){
			draggedItem.GetComponent<RectTransform>().localPosition = UIManager_craft.instance.ScreenToCanvasPoint(Input.mousePosition);
		}
				...
				foreach(var result in raycastResults){
					...
					if(result.gameObject.CompareTag("ItemIcon")){
						...
						//Stack items (IF THEY ARE THE SAME)
							...
							result.gameObject.GetComponent<Item_craft>().quantity += draggedItem.GetComponent<Item_craft>().quantity;
							result.gameObject.transform.Find("QuantityText").GetComponent<Text>().text = result.gameObject.GetComponent<Item_craft>().quantity.ToString();
